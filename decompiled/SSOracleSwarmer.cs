	...
	public override void Update(bool eu)
	{
		...
		else if (mode == MovementMode.SuckleMycelia)
		{
			...
			else if (attachedToMyc)
			{
				...
				Vector2 vector = direction * ((2f - num) * 0.15f);
				base.firstChunk.vel -= vector;
				base.firstChunk.pos -= vector;
				Vector2 vector2 = direction * ((2f - num) * 0.35f);
				suckleMyc.points[suckleMyc.points.GetLength(0) - 1, 0] += vector2;
				suckleMyc.points[suckleMyc.points.GetLength(0) - 1, 2] += vector2;
		...
		{
			Vector2 vector3 = currentBehavior.color;
			currentBehavior = new Behavior(this);
			if (UnityEngine.Random.value > 0.25f)
			{
				currentBehavior.color = vector3;
			}
		}
