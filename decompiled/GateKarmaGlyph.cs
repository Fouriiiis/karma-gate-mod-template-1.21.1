...
public class GateKarmaGlyph : CosmeticSprite
{
	...
	public Color GetToColor
	{
		get
		{
			if (PlayNoEnergyAnimation || (animationFinished && ShouldPlayCitizensIDAnimation() < 0))
			{
	...
	public bool PlayNoEnergyAnimation
	{
		get
		{
			if (ModManager.Watcher && gate.room.game.session is StoryGameSession && gate.room.game.StoryCharacter == WatcherEnums.SlugcatStatsName.Watcher)
			{
				List<Player> list = gate.ListOfPlayersInZone();
				for (int i = 0; i < list.Count; i++)
				{
					if (list[i].maxRippleLevel >= 1f)
					{
						return false;
					}
				}
			}
			return !gate.EnergyEnoughToOpen;
		}
	}
	...
	public GateKarmaGlyph(bool side, RegionGate gate, RegionGate.GateRequirement requirement)
	{
		if (requirement == null)
		{
			throw new Exception($"GateKarmaGlyph is being constructed with null requirement! It's not supported! Fix it now! side:{side} gate:{gate.room.world.region.name}");
		}
		this.side = side;
		this.gate = gate;
		...
		for (int i = 0; i < citizensIDSequence.Length; i++)
		{
			...
			{
				citizensIDSequence[i] = UnityEngine.Random.Range(0, 14);
			}
		}
	}
	...
	public override void Update(bool eu)
	{
		...
		if (gate.mode == RegionGate.Mode.MiddleClosed || gate.mode == RegionGate.Mode.Closed || gate.mode == RegionGate.Mode.Broken)
		{
			if (gate.letThroughDir == side || gate is ElectricGate)
			{
				...
				{
					goalFade = ((!PlayNoEnergyAnimation) ? Mathf.InverseLerp((gate is ElectricGate) ? 10f : 40f, 0f, gate.startCounter) : 0.82f);
				}
			}
			else
			{
				goalFade = Mathf.Min((!PlayNoEnergyAnimation) ? 1f : 0.82f, goalFade + 1f / 30f);
			}
		}
		else
		{
		...
		if (UnityEngine.Random.value < 1f / ((flicker == 0f) ? Mathf.Lerp(30f, 780f, goalFade) : 30f))
		{
			flicker = UnityEngine.Random.value;
		}
		if (UnityEngine.Random.value < 1f / 70f && PlayNoEnergyAnimation)
		{
		...
		if (ModManager.MSC && ShouldPlayCitizensIDAnimation() != 0)
		{
			...
			if (room.game.GetStorySession.saveState.hasRobo)
			{
				...
				if (animationTicker % 15 == 0)
				{
					...
					if (glyphIndex < 10)
					{
						room.PlaySound(MoreSlugcatsEnums.MSCSoundID.Data_Bit, pos, 1f, 0.5f + UnityEngine.Random.value * 2f);
					}
				}
				if (animationIndex > 9)
				{
	...
	public override void DrawSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
	{
		...
		sLeaser.sprites[0].alpha = num * Mathf.Lerp(0.55f, 0.6f, UnityEngine.Random.value);
		...
		if (ModManager.MSC && requirement == MoreSlugcatsEnums.GateRequirement.RoboLock)
		{
			...
			for (int j = 2; j < 11; j++)
			{
				...
				{
					sLeaser.sprites[j].element = Futile.atlasManager.GetElementWithName("TinyGlyph" + UnityEngine.Random.Range(0, 14));
		...
		MapExporter.GateKarmaGlyph_DrawSprites(this, sLeaser);
	...
	{
		if (requirement != MoreSlugcatsEnums.GateRequirement.RoboLock || gate.mode != RegionGate.Mode.MiddleClosed || PlayNoEnergyAnimation || gate.unlocked || gate.letThroughDir == side)
		{
			return 0;
		}
		if (ModManager.Watcher && gate.room.game.session is StoryGameSession && gate.room.game.StoryCharacter == WatcherEnums.SlugcatStatsName.Watcher)
	...
	public int ShouldPlayCitizensIDAnimation()
	{
		...
		if (gate.mode != RegionGate.Mode.MiddleClosed || PlayNoEnergyAnimation || gate.unlocked || gate.letThroughDir == side)
		{
			return 0;
		}
		int num = gate.PlayersInZone();
